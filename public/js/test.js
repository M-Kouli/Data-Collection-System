    ...
    if (type === 'Category') {
      ...
      const traces = boardNumbers.map(boardId => {
        ...
        return {
          ...
          text: boardData.map(d => formatTimestampLong(d.timestamp)), // Add timestamps for hover info
      ...
      if (option === 'All') {
        ...
        const traces = boardOptions.map(opt => ({
          x: newData.filter(d => d[opt] !== undefined).map(d => formatTimestampShort(d.timestamp)),
          ...
          text: newData.map(d => formatTimestampLong(d.timestamp)), // Add timestamps for hover info
          hoverinfo: 'y+text', // Display x, y, and timestamp on hover
          connectgaps: true
        }));
        ...
        const upperControlLimit = 255; // Replace with your actual upper control limit value
        const lowerControlLimit = 145; // Replace with your actual lower control limit value
        console.log(chartData)
        const filteredData = chartData.filter(d => d[normalizedOption] !== undefined);
        const trace = {
          x: filteredData.map(d => formatTimestampShort(d.timestamp)),
          ...
          text: filteredData.map(d => formatTimestampLong(d.timestamp)), // Add timestamps for hover info
          hoverinfo: 'y+text', // Display x, y, and timestamp on hover
          connectgaps: true
        };
